#!/usr/bin/env python3
"""
Fetch US state job openings rate from BLS Public Data API v2 (JOLTS).

Series ID structure: JTS{industry}{FIPS}{area}{size}{data_type}
  Example: JTU000000060000000JOR (California)
...
def make_series_id(fips: str) -> str:
    """Build JOLTS seasonally adjusted job openings rate series ID from state FIPS."""
    return f"JTS000000{fips}0000000JOR"
...
def parse_bls_response(
    ...
    state, year, month, date, job_openings_rate
    """
...
def fetch_jobopenings_50_states(cfg: BLSConfig) -> pd.DataFrame:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch 50-state job openings rate (JOLTS) from BLS API v2."
    )
    ...
    parser.add_argument("--out",    default="bls_jobopenings_50states.csv", help="Output CSV path")
    parser.add_argument("--master", default="bls_master.csv",               help="Master CSV path")
    args = parser.parse_args()

    cfg = BLSConfig(start_year=str(args.start), end_year=str(args.end), api_key=args.key)

    df = fetch_jobopenings_50_states(cfg)
    if df.empty:
