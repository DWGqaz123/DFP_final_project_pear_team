#!/usr/bin/env python3
"""
Fetch US state employment rates from BLS Public Data API v2 (LAUS).

- Pulls 50 states employment rate series IDs (LAUST{FIPS}0000000000007)
...
Usage:
  python3 bls_employment_rate_50states.py --start 2022 --end 2025 --key 963729bfa50042e294f9e0516067fcb7 --out employment.csv
"""
...
def make_series_id(fips: str) -> str:
    """Build LAUS employment rate series ID from a state FIPS code."""
    return f"LAUST{fips}0000000000007"
...
def parse_bls_response(
    ...
    state, series_id, year, month, period, period_name, employment_rate, date
    """
...
def fetch_employment_50_states(cfg: BLSConfig) -> pd.DataFrame:
...
def main():
    parser = argparse.ArgumentParser(description="Fetch 50-state employment rates from BLS API v2.")
    ...
    parser.add_argument("--out", default="bls_employment_50states.csv", help="Output CSV path")
    args = parser.parse_args()

    cfg = BLSConfig(start_year=str(args.start), end_year=str(args.end), api_key=args.key)

    df = fetch_employment_50_states(cfg)
    if df.empty:
    ...
    upsert_to_master(df, metric_col="employment_rate", master_path=args.master)
