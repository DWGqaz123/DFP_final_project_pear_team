#!/usr/bin/env python3
"""
Fetch US state average weekly hours of all employees from BLS Public Data API v2 (SAE/SMU).

- Pulls 50 states average weekly hours series IDs (SMU{FIPS}000000500000002)
...
def make_series_id(fips: str) -> str:
    """Build SMU average weekly hours series ID from a state FIPS code."""
    return f"SMU{fips}000000500000002"
...
def parse_bls_response(
    ...
    state, series_id, year, month, period, period_name, avg_weekly_hours, date
    """
...
def fetch_weeklyhours_50_states(cfg: BLSConfig) -> pd.DataFrame:
...
def main():
    parser = argparse.ArgumentParser(description="Fetch 50-state average weekly hours from BLS API v2.")
    ...
    parser.add_argument("--out", default="bls_weeklyhours_50states.csv", help="Output CSV path")
    args = parser.parse_args()

    cfg = BLSConfig(start_year=str(args.start), end_year=str(args.end), api_key=args.key)

    df = fetch_weeklyhours_50_states(cfg)
    if df.empty:
        print("No data returned.")
        return

    df.to_csv(args.out, index=False)
    upsert_to_master(df, metric_col="avg_weekly_hours", master_path=args.master)
